...
<vscode-collapsible class="collapsible-block"  title={features[0].category}>
	{#each features as feature}
		...
		{#if selected.includes(feature.define)}
			<vscode-checkbox bind:this={checkbox[feature.define]} value={feature.define} checked>{feature.description}</vscode-checkbox>
		{:else}
			<vscode-checkbox bind:this={checkbox[feature.define]} value={feature.define}>{feature.description}</vscode-checkbox>
		{/if}
		</div>
	{/each}
</vscode-collapsible>
