...
<script lang="ts">
	import type { TasksList } from '../tasksLists';
	...
	$effect(() => {
		Object.keys(checkbox).forEach((key) => {
			checkbox[key].addEventListener('change', () => {
				...
				var mainCheckbox = document.getElementById(features[0].category);
				if (mainCheckbox) {
...
<vscode-checkbox class="feature-checkbox" bind:this={mainCheckbox}></vscode-checkbox>
<vscode-collapsible class="collapsible-block"  title={features[0].category}>
	{#each features as feature}
		<div style="padding:2px">
		{#if selected.includes(feature.define)}
			<vscode-checkbox bind:this={checkbox[feature.label]} value={feature.define} checked>{feature.description}</vscode-checkbox>
		{:else}
			<vscode-checkbox bind:this={checkbox[feature.label]} value={feature.define}>{feature.description}</vscode-checkbox>
		{/if}
		</div>
	{/each}
</vscode-collapsible>
