<script lang="ts">
	...
	async function loadSITLOptions() {
		if (!vscodeHooks) return;
		
		try {
			const response = await vscodeHooks.request("getSITLOptions");
			options = response.options || [];
		} catch (error) {
			console.error("Failed to load SITL options:", error);
			options = [];
		}
	}
	...
	function handleInput(event: Event) {
		...
		if (currentWord.length > 0) {
			const filtered = options.filter(opt => {
				...
				// For combined options like "-A, --sitl-instance-args", check both parts
	...
	function selectOption(option: { name: string; description: string }) {
		...
		// For combined options like "-A, --sitl-instance-args", determine which part to use
...
<div class="autocomplete-container">
	...
	<div class="help-text">
		<small>
			💡 Start typing a SITL option (e.g., <code>--speedup</code> or <code>-v</code>) for autocomplete suggestions. 
