...
suite('apActions Test Suite', () => {
	...
	suite('loadDefaultActiveConfiguration Tests', () => {
		// No need for setup/teardown since we're using the main sandbox

		test('should load saved active configuration from settings', async () => {
			const mockTasks = [
				{
					definition: {
						type: 'ardupilot',
			...
			// Mock workspace configuration to return saved active configuration
			sandbox.stub(vscode.workspace, 'getConfiguration').callsFake((section?: string) => {
			...
			// Mock fetchTasks to return our test tasks
			...
			// Verify that the correct configuration was loaded
		...
		test('should use first available task when no saved configuration', async () => {
			const mockTasks = [
				{
					definition: {
						type: 'ardupilot',
			...
			// Mock workspace configuration to return no saved configuration
			sandbox.stub(vscode.workspace, 'getConfiguration').callsFake((section?: string) => {
				if (section === 'ardupilot') {
					return {
						get: () => undefined
			...
			// Verify that the first available task was loaded
			assert.ok(activeConfiguration);
		...
		test('should handle saved configuration that no longer exists', async () => {
			const mockTasks = [
				{
					definition: {
						type: 'ardupilot',
			...
			// Mock workspace configuration to return a saved configuration that doesn't exist
			sandbox.stub(vscode.workspace, 'getConfiguration').callsFake((section?: string) => {
				if (section === 'ardupilot') {
					return {
						get: (key: string) => {
							if (key === 'activeConfiguration') {
								return 'sitl-copter'; // This doesn't exist in mockTasks
							}
							return undefined;
						}
					};
			...
			// Verify that the first available task was loaded as fallback
			assert.ok(activeConfiguration);
			assert.strictEqual(activeConfiguration.definition.configure, 'CubeOrange');
			assert.strictEqual(activeConfiguration.definition.target, 'plane');
		});
